  ...
  useEffect(() => {
    const generateAndFetchSchedule = async () => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        for (
          let minOfDay = timeToMinutes("07:00");
          minOfDay < timeToMinutes("22:00");
          minOfDay += 60
        ) {
          ...
          let availablePool = Object.keys(employees).filter((empName) => {
            ...
            const isBlocked = blockedIntervals[empName]?.some(
              (interval) =>
            ...
            return (
              ...
              minOfDay >= startMin &&
              minOfDay < endMin &&
              !(minOfDay >= lunchStartMin && minOfDay < lunchEndMin)
            );
          });

          const slotTime = minOfDay / 60;
          let coverageNeeds = {};
          if (slotTime >= 8 && slotTime < 17)
            coverageNeeds = rules.coverage.primary.requirements;
          else if (slotTime >= 17)
            coverageNeeds = rules.coverage.evening.requirements;
          ...
          tasksInOrder.forEach((task) => {
            ...
            for (const empName of tempPool) {
              if (
                assignedForThisTask < needed &&
                employees[empName].abilities.includes(task)
              ) {
          ...
          for (const empName of availablePool) {
            dailyAssignments[empName][timeStr] =
              employees[empName].specialistTask;
          }
        }
        ...
        for (const empName in dailyAssignments) {
          const slots = Object.keys(dailyAssignments[empName]).sort(
            ...
            if (task !== currentTask) {
              ...
              if (currentTask) {
                finalEvents.push({
                  ...
                  resourceId: empName,
                  title: currentTask,
                  start: `${dateString}T${eventStart}:00`,
                  end: `${dateString}T${eventEnd}:00`,
                  backgroundColor: getTaskColor(currentTask),
                  borderColor: getTaskColor(currentTask),
                });
              }
              currentTask = task;
              eventStart = slot;
